    ...
    def navbar(self, prefix='Welcome', action=None,
        ...
        if self.user_id:  # User is logged in
            ...
            if 'impersonate' not in self.settings.actions_disabled:
                if self.has_permission('impersonate', self.table_user(), self.user_id): items.append({'name': T('Impersonate'), 'href': href('impersonate'), 'icon': 'icon-lock'})
                elif self.is_impersonating(): items.append({'name': T('Depersonate'), 'href': href('impersonate/0'), 'icon': 'icon-lock'})
